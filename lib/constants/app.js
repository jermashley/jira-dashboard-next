...
export const STATUS_GROUP_QA = [STATUS_NEEDS_QA, STATUS_READY_FOR_QA, STATUS_IN_QA]
export const STATUS_GROUP_DEPLOYMENT = [STATUS_NEEDS_DEPLOYMENT, STATUS_DEPLOYED, STATUS_DONE]
...
export const statusGroupColor = (status) => {
  ...
  if (STATUS_GROUP_DEVELOPMENT.includes(status)) {
    return `blue-600`
  }

  if (STATUS_GROUP_QA.includes(status)) {
    return `indigo-500`
  }
  if (STATUS_GROUP_DEPLOYMENT.includes(status)) {
    return `green-500`
  }
