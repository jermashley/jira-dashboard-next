...
const DEFAULT_SOFTWARE = [SOFTWARE_PIPELINE, SOFTWARE_HIVE, SOFTWARE_AIR2POST, SOFTWARE_PANGEA]
const DEFAULT_RECENT_DAYS = 15
const QA_PROCESS_STATUSES = [`Needs QA`, `Ready for QA`, `In QA`, `Needs Deployment`]
...
const sanitizeData = (data) => {
  ...
  if (Array.isArray(data)) {
    ...
    if (data.length >= 2) {
      const sanitized = data.map((value) => `"${value}"`)
...
export const recentDeployments = (
  ...
  `project in (${sanitizeData(project)}) AND software in (${sanitizeData(
    software
 ...
 * @param {array} software
 */
export const criticalIssues = (project = DEFAULT_PROJECTS, software = DEFAULT_SOFTWARE) =>
  `project in (${sanitizeData(project)}) AND software in (${sanitizeData(
    software
 ...
 * @param {array} software
 */
export const urgentIssues = (project = DEFAULT_PROJECTS, software = DEFAULT_SOFTWARE) =>
  `project in (${sanitizeData(project)}) AND software in (${sanitizeData(
    software
 ...
 * @param {array} software
 */
export const qaIssues = (project = DEFAULT_PROJECTS, software = DEFAULT_SOFTWARE) =>
  `project in (${sanitizeData(project)}) AND software in (${sanitizeData(
    software
  )}) AND status in (${sanitizeData(QA_PROCESS_STATUSES)}) ORDER BY created DESC`
 ...
 * @param {array} software
 */
export const defaultIssues = (project = DEFAULT_PROJECTS, software = DEFAULT_SOFTWARE) =>
  `project in (${sanitizeData(project)}) AND software in (${sanitizeData(
    software
...
export const stagingTestingServer = (server) =>
  `project in (${sanitizeData(
    DEFAULT_PROJECTS
